...
These provide:
- =libmp3lame= — MP3 encoding (via LAME, CFFI bindings in cl-streamer)
- =libfdk-aac= — AAC encoding (via FDK-AAC, C shim in cl-streamer)
- =libflac= — FLAC decoding (via cl-mixed-flac)
- =libmpg123= — MP3 decoding (via cl-mixed-mpg123)
- =libtagc= — audio metadata extraction (via taglib)
- =libfixposix= — required by iolib (socket networking for CL-Streamer)
...
**** Audio Pipeline (CL-Streamer)
- =cl-streamer= — custom HTTP audio streaming server (replaces Icecast)
- =cl-streamer/harmony= — Harmony audio backend (replaces Liquidsoap)
- =cl-streamer/encoder= — MP3 encoding via LAME (CFFI)
- =cl-streamer/aac-encoder= — AAC encoding via FDK-AAC (C shim + CFFI)
- =harmony= — audio framework (decode, mix, crossfade)
- =cl-mixed= / =cl-mixed-flac= / =cl-mixed-mpg123= — low-level audio mixing and decoding
- =cffi= — C foreign function interface (LAME, FDK-AAC bindings)
- =iolib= — I/O library (sockets with TCP_NODELAY, SO_KEEPALIVE for CL-Streamer)
...
- =cl-cron= - Cron-style playlist scheduling
- =lparallel= - Parallel computation
- =flexi-streams= - Flexible stream types
- =split-sequence= - Sequence splitting
...
sudo apt update
sudo apt install -y sbcl git curl gcc make postgresql postgresql-client

# Install audio library dev packages (required for building cl-streamer)
sudo apt install -y \
  libmp3lame-dev libfdk-aac-dev \
  libflac-dev libmpg123-dev \
  libtag1-dev libfixposix-dev
...
# Note: PostgreSQL can run natively or in Docker
...
# Build the FDK-AAC shim (first time only, or after updating libfdk-aac)
cd cl-streamer && gcc -shared -fPIC -o libfdkaac-shim.so fdkaac-shim.c -lfdk-aac && cd ..
...
- *MP3 Stream*: http://localhost:8000/asteroid.mp3 (CL-Streamer)
- *AAC Stream*: http://localhost:8000/asteroid.aac (CL-Streamer)
- *Slynk REPL*: port 4009 (connect with Sly/SLIME for live development)
