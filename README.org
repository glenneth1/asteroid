...
** System Libraries
- =libmp3lame= - MP3 encoding (LAME, via CFFI)
- =libfdk-aac= - AAC encoding (FDK-AAC, via C shim + CFFI)
...
- =libfixposix= - POSIX compatibility (required by iolib for socket networking)
- =gcc= - C compiler (for building the FDK-AAC shim)
...
- No Icecast or Liquidsoap required — CL-Streamer replaces both
